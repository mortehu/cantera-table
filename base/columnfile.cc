...
ColumnFileWriter::ColumnFileWriter(kj::AutoCloseFd&& fd) : fd_(std::move(fd)) {
  WriteAll(fd_, StringRef(kMagic, sizeof(kMagic)));
}
...
ColumnFileWriter::ColumnFileWriter(std::string& output)
  ...
  output_string_->append(kMagic, sizeof(kMagic));
}
...
ColumnFileReader::FieldReader::FieldReader(std::string&& buffer,
                                           ColumnFileCompression compression)
  ...
  if (!repeat_) {
    ...
    if ((b0 & 0xc0) == 0xc0) {
      ...
      if (b0 == 0xff) {
        ...
        // The value we're about to readvalue shares a prefix at least 2 bytes
        // long with the previous value.
...
void ColumnFileReader::Fill() {
  ...
  if (fd_ != nullptr) {
    for (const auto& f : field_sizes) {
      std::string buffer;
      buffer.resize(f.second);
      Read(fd_, &buffer[0], f.second, f.second);
      fields_.emplace_back(f.first,
                           FieldReader(std::move(buffer), compression));
    }
  } else {
    for (const auto& f : field_sizes) {
