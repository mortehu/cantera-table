...
void WriteAll(int fd, const StringRef& buffer) {
  ...
  while (offset < buffer.size()) {
    ssize_t ret;
    KJ_SYSCALL(ret = write(fd, data + offset, buffer.size() - offset));
    offset += ret;
  }
}
...
void PRead(int fd, void* dest, size_t size, off_t offset) {
  while (size > 0) {
    ssize_t ret;
    KJ_SYSCALL(ret = pread(fd, dest, size, offset));
    KJ_REQUIRE(ret > 0, "unexpectedly reached end of file", offset, size);
    size -= ret;
    offset += ret;
    dest = reinterpret_cast<char*>(dest) + ret;
  }
...
kj::Array<char> ReadFD(int fd) {
  ...
  if (size == -1) {
    ...
    // The lambda will keep the buffer alive due to its copy of the
    // std::shared_ptr.
