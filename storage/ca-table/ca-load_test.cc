...
struct CaLoadTest : testing::Test {
 ...
 protected:
  ...
  void Merge(const std::string& output, const std::vector<std::string>& inputs,
    ...
    args.emplace_back("storage/ca-table/ca-load");
...
TEST_F(CaLoadTest, AddsPrefixToSingleTable) {
  auto table_00_path = (temp_directory_->Root() + "/table_00");
  auto table_01_path = (temp_directory_->Root() + "/table_01");

  ca_offset_score v;
  memset(&v, 0, sizeof(v));

  auto table_00 = ca_table_open("leveldb-table", table_00_path.c_str(),
  ...
  auto table_01 =
      ca_table_open("leveldb-table", table_01_path.c_str(), O_RDONLY);
...
TEST_F(CaLoadTest, AddsPrefixToMergedTables) {
  auto table_00_path = (temp_directory_->Root() + "/table_00");
  auto table_01_path = (temp_directory_->Root() + "/table_01");
  auto table_02_path = (temp_directory_->Root() + "/table_02");
  ...
  auto table_01 = ca_table_open("leveldb-table", table_01_path.c_str(),
                                O_CREAT | O_TRUNC | O_WRONLY);
  v.offset = 2;
  v.score = 4.0f;
  ...
  auto table_02 =
      ca_table_open("leveldb-table", table_02_path.c_str(), O_RDONLY);
