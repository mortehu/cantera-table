...
//              key_offsets.
void ProcessRange(const ev::StringRef& key,
  ...
  if (!ca_table::Keywords::GetInstance().IsTimestamped(key)) {
    min_score_string = ev::FloatToString(min_score);
    max_score_string = ev::FloatToString(max_score);
  } else {
    if (do_timestamps) {
      min_score_string = ev::cat(ev::FloatToString(min_score), " days ago");
      max_score_string = ev::cat(ev::FloatToString(max_score), " days ago");
    } else {
...
std::string PrimaryKeywordForQuery(ev::StringRef query) {
  const char kDelimiters[] = " <>[";

  for (const auto delim : kDelimiters) {
    query.ConsumeTail(query.end() - query.find(delim));
  }

  if ((HasPrefix(query, "min(") || HasPrefix(query, "max(")) &&
      HasSuffix(query, ")")) {
    query.Consume(4);
    query.pop_back();
  }
...
void ca_schema_query_correlate(struct ca_schema* schema, const char* query_A,
                               const char* query_B) {
  const auto& keywords = ca_table::Keywords::GetInstance();
  ...
  ca_table::Query(offsets_A, query_A, index_tables, false, false);
  ca_table::Query(offsets_B, query_B, index_tables, false, false);

  offsets_B.resize(ca_table::SubtractOffsets(&offsets_B[0], offsets_B.size(),
  ...
  // interested in it.
  const auto limit_A = offsets_A.size() / 20;
  const auto limit_B = offsets_B.size() / 20;
  ...
  for (auto& index_table : index_tables) {
    index_table->Seek(0, SEEK_SET);

    struct iovec key_iov, data_iov;

    while (1 == index_table->ReadRow(&key_iov, &data_iov)) {
      if (a_is_timestamped && keywords.IsEphemeral(key_iov)) continue;

      key_offsets.clear();

      ca_offset_score_parse(data_iov, &key_offsets);
