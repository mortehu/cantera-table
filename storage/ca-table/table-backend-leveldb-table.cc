...
class LevelDBTable : public ca_table::Table {
 public:
  ...
  void Sync() override {
    ...
    ev::LinkAnonTemporaryFile(temp_fd_, path_.c_str());
  }
...
std::unique_ptr<Table> LevelDBTableBackend::Open(const char* path, int flags,
  ...
  if ((flags & (O_CREAT | O_TRUNC | O_WRONLY)) ==
    ...
    std::string temp_dir(".");
    if (auto last_slash = strrchr(path, '/'))
      temp_dir = std::string(path, last_slash);
    auto temp_file = ev::AnonTemporaryFile(temp_dir.c_str());
  ...
  if (flags == O_RDONLY) {
    auto file_object =
        std::make_unique<LevelDBFile>(ev::OpenFile(path, O_RDONLY | O_CLOEXEC));
