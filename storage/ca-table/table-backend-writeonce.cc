...
class WriteOnceTable : public ca_table::Table {
 public:
  ...
  bool SeekToKey(const ev::StringRef& key) override;

  off_t Offset() override;

  ssize_t ReadRow(struct iovec* key, struct iovec* value) override;
...
uint64_t CA_wo_hash(const ev::StringRef& str) {
...
void WriteOnceTable::BuildIndex() {
  ...
  for (;;) {
    ...
    if (1 != ReadRow(&key_iov, &value)) break;
...
void WriteOnceTable::Sync() {
  ...
  header.data_crc32c = crc32c;

  KJ_SYSCALL(lseek(fd, 0, SEEK_SET), tmp_path);

  ev::WriteAll(fd, ev::StringRef(reinterpret_cast<const char*>(&header),
                                 sizeof(header)));
...
bool WriteOnceTable::SeekToKey(const ev::StringRef& key) {
...
ssize_t WriteOnceTable::ReadRow(struct iovec* key, struct iovec* value) {
  uint64_t size;
  uint8_t* p;
  ...
  if (offset_ >= header->index_offset || *p == 0) {
    return 0;
  }
